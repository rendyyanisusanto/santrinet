...
class kategori_barang extends MY_Controller {
	public $arr = [
			'title'				=>	'Master Data Kategori Barang',
			...
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/general/page/kategori_barang'
	];
	...
	function print_web(){
		...
		$this->load->view('role/general/page/kategori_barang/print_page/print_web', $data);
	}
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/general/page/kategori_barang/print_page/cetak_pdf',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
	...
	function simpan_data()
	{
		try {
			...
			if ($this->save_data('kategori_barang', $data)) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data kategori_barang berhasil ditambahkan'
				]);
			}else{
				echo json_encode([
					'status'	=>	500,
					'msg'		=>	'Data kategori_barang gagal ditambahkan'
				]);
			}
		} catch (Exception $e) {
			echo json_encode([
	...
	function update_data()
	{
		...
		try {
			...
			if ($this->my_update('kategori_barang', $data, [$this->arr['id'] => $_POST['id']])) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data kategori_barang berhasil ditambahkan'
				]);
	...
	public function import_data()
	{
		try {
			...
			foreach ($_POST['dt'] as $key => $value) {
				...
				try {
					$this->save_data('kategori_barang', $send);					
				} catch (Exception $e) {
					break;
				}
			}
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            ...
            $row[]		=	'<a href="kategori_barang/edit_page/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a>';
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="kategori_barang/look_page/'.$field['id'].'" class="app-item"><i class="icon-eye"></i> Lihat</a></li>
            							<li><a href="kategori_barang/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>
