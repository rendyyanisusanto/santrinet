...
class mutasi_bahan extends MY_Controller {
	public $arr = [
			'title'				=>	'Mutasi Bahan',
			'table'				=>	'mutasi_bahan',
			'column'			=>	[ 'kode','tanggal', 'keterangan', 'status','created_at','created_by'],
			'column_order'		=>	[ 'id','kode','tanggal', 'keterangan',  'status','created_at','created_by'],
			'column_search'		=>	[ 'id','kode','tanggal', 'keterangan',  'status','created_at','created_by'],
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/admin/page/mutasi_bahan'
	];
	...
	function edit_page($id = ""){
		try {
			if ($id !=='') {
				...
				$data['satuan_beli']			=	$this->my_where('satuan_beli',['status'=>1])->result_array();
				$data['satuan_pakai']		=	$this->my_where('satuan_pakai',[])->result_array();
				$data['data_edit']['po']	=	$this->my_where('mutasi_bahan', ['id'=>$id])->row_array();
				$data['data_edit']['detail']=	$this->my_where('v_detail_mutasi_bahan', ['mutasi_bahan_id'=>$id])->result_array();
	...
	function print_inv($id){
		if (!empty($id)) {
			...
			$data['notaheader']		=	$this->get_setting_table('mutasi_bahan', 'template_nota_header');
			$data['notafooter']		=	$this->get_setting_table('mutasi_bahan', 'template_nota_footer');
			$data['detail_mutasi_bahan']	=	$this->db->query('select *, (select nama from master_data_bahan where master_data_bahan.id = bahan_id) as nama_bahan, (select nama from satuan_beli where satuan_beli.id=satuan_beli_id) as nama_satuan from detail_mutasi_bahan where mutasi_bahan_id = '.$id)->result_array();
			$data['slice']	=	[];
			$n = 0;
			$i = 0;
			foreach ($data['detail_mutasi_bahan'] as $key => $value) {
				if ($i<=10) {
					$data['slice'][$n]['detail_mutasi_bahan'][] = $value;
				}else{
					$n++;
					$i=0;
				}
	...
	function print_struk($id){
		try {
			if($id !== ""){
				$data['param'] 		= 	$this->arr;
				$data['account']	=	$this->get_user_account();
				$data['mutasi_bahan']		=	$this->my_where('mutasi_bahan',['id'=>$id])->row_array();
				$data['master_data_suplier']	=	$this->my_where('master_data_suplier',['id'=>$data['mutasi_bahan']['suplier_id']])->row_array();
				$data['detail_mutasi_bahan']	=	$this->db->query('select *, (select nama from master_data_bahan where master_data_bahan.id = bahan_id) as nama_bahan, (select nama from satuan_beli where satuan_beli.id=satuan_beli_id) as nama_satuan from detail_mutasi_bahan where mutasi_bahan_id = '.$id)->result_array();
				
				$data['struk']		=	$this->get_setting_table('mutasi_bahan', 'template_struk_header');
				$data['strukfooter']		=	$this->get_setting_table('mutasi_bahan', 'template_struk_footer');
	...
	function print_web(){
		...
		$this->load->view('role/admin/page/mutasi_bahan/print_page/print_web', $data);
	}
	function print_excel(){
		...
		$path_exc = $this->my_where('setting_table', ['table'=>'mutasi_bahan','name'=>'template_excel'])->row_array();      
        $start_no = $this->my_where('setting_table', ['table'=>'mutasi_bahan','name'=>'start_row_excel'])->row_array();        
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/admin/page/mutasi_bahan/print_page/print_web',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
			           	"mutasi_bahan"		=>	$data[$data['param']['table']],
			           	"param"		=>	$this->arr
	     			],
	...
	function simpan_data()
	{
		try {
			...
			if ($this->save_data('mutasi_bahan', $data)) {
				$data_mutasi = $this->my_where('mutasi_bahan', [
					'kode' => $_POST['kode'],
					'tanggal' => $_POST['tanggal'],
				])->row_array();
				...
				foreach ($_POST['data'] as $value) {
					$data_detail = [
						'mutasi_bahan_id' => $data_mutasi['id'],
						'bahan_id' => $value['bahan_id'],
						'qty' => $value['qty'],
						'satuan_beli_id' => $value['satuan_id'],
					];
				
					$this->save_data('detail_mutasi_bahan', $data_detail);
				}
	...
	function update_data()
	{
		try {
			...
			if ($this->my_update('mutasi_bahan', $data, [$this->arr['id'] => $_POST['id']])) {
				$data_po = $this->my_where('mutasi_bahan',[$this->arr['id'] => $_POST['id']])->row_array();

				foreach ($_POST['data'] as $value) {
					$data_detail = [
						'mutasi_bahan_id' => $data_po['id'],
						'bahan_id' => $value['bahan_id'],
						'qty' => $value['qty'],
						'satuan_beli_id' => $value['satuan_id'],
					];
					if (isset($value['detail_po_id'])) {
						$this->my_update('detail_mutasi_bahan', $data_detail, ['id' => $value['detail_po_id']]);
					}else{
						$this->save_data('detail_mutasi_bahan', $data_detail);
					}
	...
	function delete_row(){
		try {
			if(isset($_POST['id'])){
				...
				if($this->db->delete('detail_mutasi_bahan', ['id'=>$_POST['id']])){
					echo json_encode([
						'status'=>200,
	...
	public function import_data()
	{
		try {
			...
			foreach ($_POST['dt'] as $key => $value) {
				...
				try {
					$this->save_data('mutasi_bahan', $send);					
				} catch (Exception $e) {
					break;
				}
			}
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            ...
            $row[]		=	'<a href="mutasi_bahan/invoice/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a>';
            // $row[]		=	$field['qty'];
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="mutasi_bahan/invoice/'.$field['id'].'" class="app-item"><i class="icon-eye"></i> Lihat</a></li>
            							'.(($field['status']==1)?'<li><a href="mutasi_bahan/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>':'').'
            						</ul>
            					</li>
            				</ul>';
	...
	function cancel_item(){
		...
		try {
			...
			if ($id !== '') {
				$data['po']	=	$this->my_where('mutasi_bahan', ['id'=>$id])->row_array();
				$data['pb']	=	$this->db->query('SELECT *, (select count(*) from v_detail_pb_pt where pb_id=penerimaan_barang.id and status_aktif=1) as jml_pt FROM `penerimaan_barang` where mutasi_bahan_id='.$id.' and status_aktif=1')->result_array();
				$data['pt']	=	$this->my_where('v_detail_pb_pt', ['mutasi_bahan_id'=>$id, 'status_aktif'=>1])->result_array();
	...
	function cek_cancel_item(){
		try {
			$id = $_POST['id'];
			$pb	=	$this->db->query('SELECT *, (select count(*) from v_detail_pb_pt where pb_id=penerimaan_barang.id and status_aktif=1) as jml_pt FROM `penerimaan_barang` where mutasi_bahan_id='.$id.' and status_aktif=1')->num_rows();
			$pt	=	$this->my_where('v_detail_pb_pt', ['mutasi_bahan_id'=>$id, 'status_aktif'=>1])->num_rows();
	...
	function cancel_po(){
		try {
			...
			$id = $_POST['id'];
			if ($this->my_update('mutasi_bahan', ['status_aktif'=>0], ['id'=>$id])) {

				$this->my_update('jurnal_umum', ['status_aktif'=>0], ['table_id'=>$_POST['id'], 'table'=>'mutasi_bahan']);
				echo json_encode([
					'status'=>200,
					'msg' => "Berhasil membatalkan mutasi_bahan"
				]);
			}else{
				echo json_encode([
					'status'=>500,
					'msg' => "Gagal membatalkan mutasi_bahan"
				]);
			};
		} catch (Exception $e) {
