...
class gudang extends MY_Controller {
	public $arr = [
			...
			'table'				=>	'gudang',
			'column'			=>	[ 'kode','nama', 'color','status','alamat'],
			'column_order'		=>	[ 'id','kode','nama', 'color','status','alamat'],
			'column_search'		=>	[ 'id','kode','nama', 'color','status','alamat'],
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/general/page/gudang'
	];
	...
	function print_web(){
		...
		$this->load->view('role/general/page/gudang/print_page/print_web', $data);
	}
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/general/page/gudang/print_page/cetak_pdf',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
	...
	function simpan_data()
	{
		try {
			...
			$data = [
				...
				'status' => $_POST['status'],
				'alamat' => $_POST['alamat'],
				'created_at' => $this->get_user_account()['id']
			];
			if ($this->save_data('gudang', $data)) {
				...
				echo json_encode([
					'status'	=>	500,
					'msg'		=>	'Data gudang gagal ditambahkan'
				]);
			}
		} catch (Exception $e) {
			echo json_encode([
	...
	function update_data()
	{
		...
		try {
			...
			if ($this->my_update('gudang', $data, [$this->arr['id'] => $_POST['id']])) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data gudang berhasil ditambahkan'
				]);
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            ...
            $row[]		=	'<a href="gudang/edit_page/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a>';
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="gudang/look_page/'.$field['id'].'" class="app-item"><i class="icon-eye"></i> Lihat</a></li>
            							<li><a href="gudang/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>
