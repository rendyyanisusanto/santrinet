...
class status_shift extends MY_Controller {
	public $arr = [
			'title'				=>	'Master Data Status Shift',
			'table'				=>	'status_shift',
			'column'			=>	[ 'kode','nama', 'color','status', 'shift_in', 'shift_out'],
			'column_order'		=>	[ 'id','kode','nama', 'color','status', 'shift_in', 'shift_out'],
			'column_search'		=>	[ 'id','kode','nama', 'color','status', 'shift_in', 'shift_out'],
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/general/page/status_shift'
	];
	...
	function print_web(){
		...
		$this->load->view('role/general/page/status_shift/print_page/print_web', $data);
	}
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/general/page/status_shift/print_page/cetak_pdf',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
	...
	function simpan_data()
	{
		try {
			...
			$data = [
				...
				'color' => $_POST['color'],
				'status' => $_POST['status'],
				'check_in' => $_POST['shift_in'],
				'check_out' => $_POST['shift_out'],
				'created_by' => $this->get_user_account()['id']
			];
			if ($this->save_data('status_shift', $data)) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data status_shift berhasil ditambahkan'
				]);
			}else{
				echo json_encode([
					'status'	=>	500,
					'msg'		=>	'Data status_shift gagal ditambahkan'
				]);
			}
		} catch (Exception $e) {
			echo json_encode([
	...
	function update_data()
	{
		...
		try {
			...
			if ($this->my_update('status_shift', $data, [$this->arr['id'] => $_POST['id']])) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data status_shift berhasil ditambahkan'
				]);
	...
	public function import_data()
	{
		try {
			...
			foreach ($_POST['dt'] as $key => $value) {
				...
				try {
					$this->save_data('status_shift', $send);					
				} catch (Exception $e) {
					break;
				}
			}
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            ...
            $row[]		=	'<a href="status_shift/edit_page/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a>';
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="status_shift/look_page/'.$field['id'].'" class="app-item"><i class="icon-eye"></i> Lihat</a></li>
            							<li><a href="status_shift/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>
