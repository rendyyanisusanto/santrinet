...
class satuan_pakai extends MY_Controller {
	public $arr = [
			'title'				=>	'Master Data Satuan Pakai',
			'table'				=>	'satuan_pakai',
			'column'			=>	[ 'kode','nama', 'pembagi'],
			'column_order'		=>	[ 'id','kode','nama', 'pembagi'],
			'column_search'		=>	[ 'id','kode','nama', 'pembagi'],
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/general/page/satuan_pakai'
	];
	...
	function print_web(){
		...
		$this->load->view('role/general/page/satuan_pakai/print_page/print_web', $data);
	}
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/general/page/satuan_pakai/print_page/cetak_pdf',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
	...
	function simpan_data()
	{
		try {
			...
			if ($this->save_data('satuan_pakai', $data)) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data satuan_pakai berhasil ditambahkan'
				]);
			}else{
				echo json_encode([
					'status'	=>	500,
					'msg'		=>	'Data satuan_pakai gagal ditambahkan'
				]);
			}
		} catch (Exception $e) {
			echo json_encode([
	...
	function update_data()
	{
		...
		try {
			...
			if ($this->my_update('satuan_pakai', $data, [$this->arr['id'] => $_POST['id']])) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data satuan_pakai berhasil ditambahkan'
				]);
	...
	public function import_data()
	{
		try {
			...
			foreach ($_POST['dt'] as $key => $value) {
				...
				try {
					$this->save_data('satuan_pakai', $send);					
				} catch (Exception $e) {
					break;
				}
			}
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            $no++;
            $satuan_beli	=	$this->my_where('satuan_beli', ['id'=>$field['satuan_beli_id']])->row_array();
            $row        =   array();
            $row[]      =   '<input type="checkbox" onchange="bulk_checkbox('.$field['id'].')" name="get-check" value="'.$field['id'].'"></input>';
            $row[]		=	$satuan_beli['nama'];
            $row[]		=	!empty($field['nama']) ? '<a href="satuan_pakai/edit_page/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a> - <b >'.strtoupper($field['nama']).'</b>' : '-';
            $row[]		=	!empty($field['pembagi']) ? '<b >'.strtoupper($field['pembagi']).'</b>' : '-';
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="satuan_pakai/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>
