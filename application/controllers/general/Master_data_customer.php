...
class Master_data_customer extends MY_Controller {
	public $arr = [
			'title'				=>	'Master Data Customer',
			'table'				=>	'master_data_customer',
			...
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/general/page/master_data_customer'
	];
	...
	function print_web(){
		...
		$this->load->view('role/general/page/master_data_customer/print_page/print_web', $data);
	}
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/general/page/master_data_customer/print_page/cetak_pdf',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
	...
	function simpan_data()
	{
		try {
			...
			if ($this->save_data('master_data_customer', $data)) {
				echo json_encode([
					'status'	=>	200,
	...
	function update_data()
	{
		...
		try {
			...
			if ($this->my_update('master_data_customer', $data, [$this->arr['id'] => $_POST['id']])) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data customer berhasil ditambahkan'
				]);
	...
	public function import_data()
	{
		try {
			...
			foreach ($_POST['dt'] as $key => $value) {
				...
				try {
					$this->save_data('master_data_customer', $send);					
				} catch (Exception $e) {
					break;
				}
			}
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            ...
            $row[]		=	'<a href="master_data_customer/edit_page/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a>';
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="master_data_customer/look_page/'.$field['id'].'" class="app-item"><i class="icon-eye"></i> Lihat</a></li>
            							<li><a href="master_data_customer/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>
