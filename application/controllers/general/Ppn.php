...
class ppn extends MY_Controller {
	public $arr = [
			...
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/general/page/ppn'
	];
	...
	function print_web(){
		...
		$this->load->view('role/general/page/ppn/print_page/print_web', $data);
	}
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/general/page/ppn/print_page/cetak_pdf',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
	...
	function simpan_data()
	{
		try {
			...
			if ($_POST['status'] == 1) {
					$this->my_update('ppn', ['status'=> 0], []);
			}
			if ($this->save_data('ppn', $data)) {

				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data ppn berhasil ditambahkan'
				]);
			}else{
				echo json_encode([
					'status'	=>	500,
					'msg'		=>	'Data ppn gagal ditambahkan'
				]);
			}
		} catch (Exception $e) {
			echo json_encode([
	...
	function update_data()
	{
		...
		try {
			...
			if ($this->my_update('ppn', $data, [$this->arr['id'] => $_POST['id']])) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data ppn berhasil ditambahkan'
				]);
	...
	function change_status(){
		try {
			$dt = $this->arr;
			$this->my_update($dt['table'], ['status'=> 0], []);
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            ...
            $row[]		=	'<a href="ppn/edit_page/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a>';
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="ppn/look_page/'.$field['id'].'" class="app-item"><i class="icon-eye"></i> Lihat</a></li>
            							<li><a href="ppn/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>
