...
class satuan_beli extends MY_Controller {
	public $arr = [
			'title'				=>	'Master Data Satuan Beli',
			'table'				=>	'satuan_beli',
			...
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/general/page/satuan_beli'
	];
	...
	function print_web(){
		...
		$this->load->view('role/general/page/satuan_beli/print_page/print_web', $data);
	}
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/general/page/satuan_beli/print_page/cetak_pdf',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
	...
	function update_data()
	{
		...
		try {
			...
			if ($this->my_update('satuan_beli', $data, [$this->arr['id'] => $_POST['id']])) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data satuan_beli berhasil ditambahkan'
				]);
	...
	public function import_data()
	{
		try {
			...
			foreach ($_POST['dt'] as $key => $value) {
				...
				try {
					$this->save_data('satuan_beli', $send);					
				} catch (Exception $e) {
					break;
				}
			}
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            ...
            $row[]		=	'<a href="satuan_beli/edit_page/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a>';
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="satuan_beli/look_page/'.$field['id'].'" class="app-item"><i class="icon-eye"></i> Lihat</a></li>
            							<li><a href="satuan_beli/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>
