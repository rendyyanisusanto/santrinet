...
class karyawan extends MY_Controller {
	public $arr = [
			...
			'table'				=>	'karyawan',
			'column'			=>	[ 'kode','nik','uid_fingerprint','nama', 'alamat','no_hp','jk','province_id', 'status_karyawan_id', 'city_id','email', 'jabatan_id','tanggal_masuk','status','created_at','created_by','updated_at','updated_by'],
			'column_order'		=>	[ 'id','kode','nik','uid_fingerprint','nama', 'alamat', 'no_hp','jk','province_id', 'status_karyawan_id', 'city_id','email', 'jabatan_id','tanggal_masuk','status','created_at','created_by','updated_at','updated_by'],
			'column_search'		=>	[ 'id','kode','nik','uid_fingerprint','nama', 'alamat', 'no_hp','jk','province_id', 'status_karyawan_id', 'city_id','email', 'jabatan_id','tanggal_masuk','status','created_at','created_by','updated_at','updated_by'],
			'order'				=>	['id'	=>	'DESC'],
			'id'				=>	'id',
			'parents_link'		=>	'role/general/page/karyawan'
	];
	...
	public function add_page()
	{
		...
		$data['status_karyawan']	=	$this->my_where('status_karyawan',['status'=>1])->result_array();
	...
	function print_web(){
		...
		$this->load->view('role/general/page/karyawan/print_page/print_web', $data);
	}
	...
	function print_pdf()
	{
		...
		$param	=	[
	          	'url'			=>	'role/general/page/karyawan/print_page/cetak_pdf',
	          	'customPaper'	=>	'A4',
	          	'data_value'	=>	[
	...
	function simpan_data()
	{
		try {
			...
			$data = [
				...
				'nama' => $_POST['nama'],
				'no_hp' => $_POST['no_hp'],
				'uid_fingerprint' => $_POST['uid_fingerprint'],
				'jk' => $_POST['jk'],
				'province_id' => $_POST['province_id'],
				'city_id' => $_POST['city_id'],
				'alamat' => $_POST['alamat'],
				'status_karyawan_id'=>$_POST['status_karyawan_id'],
				'email' => $_POST['email'],
			...
			if ($this->save_data('karyawan', $data)) {
				echo json_encode([
					'status'	=>	200,
	...
	function update_data()
	{
		...
		try {
			...
			if ($this->my_update('karyawan', $data, [$this->arr['id'] => $_POST['id']])) {
				echo json_encode([
					'status'	=>	200,
					'msg'		=>	'Data customer berhasil ditambahkan'
				]);
	...
	public function datatable()
	{
        ...
        foreach ($list as $field) {
            ...
            $jabatan 	=	$this->my_where('jabatan', ['id'=>$field['jabatan_id']])->row_array();
            $status_karyawan 	=	$this->my_where('status_karyawan', ['id'=>$field['status_karyawan_id']])->row_array();
            $row        =   array();
            $row[]      =   '<input type="checkbox" onchange="bulk_checkbox('.$field['id'].')" name="get-check" value="'.$field['id'].'"></input>';
            $row[]		=	'<a href="karyawan/edit_page/'.$field['id'].'" class="app-item"><b>'. (!empty($field['kode']) ? strtoupper($field['kode']) : '-') . '</b></a>';
            $row[]		=	!empty($field['uid_fingerprint']) ? '<b class="text-success">'.strtoupper($field['uid_fingerprint']).'</b>': '<b class="text-danger">Belum disetting</b>';
            $row[]		=	!empty($field['nik']) ? strtoupper($field['nik']) : '-';
            $row[]		=	!empty($field['nama']) ? strtoupper($field['nama']) : '-';
            $row[]		=	!empty($field['no_hp']) ? $field['no_hp'] : '-';
            $row[]		=	!empty($field['jk']) ? $field['jk'] : '-';
            $row[]		=	!empty($field['alamat']) ? $field['alamat'] : '-';
            $row[]		=	!empty($field['city_id']) ? $province.' - '.$city : '-';
            $row[]		=	!empty($field['email']) ? $field['email'] : '-';
            $row[]		=	!empty($field['tanggal_masuk']) ? $field['tanggal_masuk'] : '-';
            $row[]		=	!empty($status_karyawan['nama']) ? '<b style="color:'.$status_karyawan['color'].';">'.$status_karyawan['nama'].'</b>' : '-';
            $row[]		=	!empty($jabatan['nama']) ? $jabatan['nama'] : '-';
            ...
            $row[]		=	'<ul class="text-center icons-list">
            					<li class="dropdown">
            						...
            						<ul class="dropdown-menu dropdown-menu-right">
            							<li><a href="karyawan/look_page/'.$field['id'].'" class="app-item"><i class="icon-eye"></i> Lihat</a></li>
            							<li><a href="karyawan/edit_page/'.$field['id'].'" class="app-item"><i class="icon-pencil"></i> Ubah</a></li>
