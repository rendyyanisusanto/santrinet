		...
		<div class="panel">
			<form id="app-submit" class="form-horizontal" action="<?php echo $data_get['param']['table'] ?>/update_data" method="POST">
				<div class="panel-body">	
					<div class="col-md-12">
						<fieldset>
							<legend class="text-semibold"> Master Data Groups - <b>EDIT DATA</b></legend>
							...
							<div class="col-md-6">
								...
								<div class="form-group">
									...
									<div class="col-lg-6">
										<input type="hidden" placeholder="" required value="<?= $data_get['data_edit']['id'] ?>" name="id" class="form-control">
										<input type="text" placeholder="" required value="<?= $data_get['data_edit']['name'] ?>" name="name" class="form-control">
									</div>
								</div>
								<div class="form-group">
									<label class="col-lg-3 control-label">Keterangan : <i class="text-danger">*</i></label>
									<div class="col-lg-6">
										<input type="text" placeholder="" required value="<?= $data_get['data_edit']['description'] ?>" name="description" class="form-control">
									</div>
								</div>
							</div>
									...
									<?php foreach ($data_get['menu'] as $key_menu => $value): ?>
										...
										<table style="width: 50%;" class="table table-framed table-bordered table-xxs">
											...
											<?php foreach ($value['submenu'] as $key => $value_sub): ?>
												...
												<tr>
													<td><input name="submenu[<?= $rand ?>][id]" type="hidden" value="<?= $value_sub['submenu']['id'] ?>"><?= $value_sub['submenu']['submenu'] ?></td>
													<td><input type="checkbox" class="chk<?= $key_menu?> c<?= $key_menu ?>" <?= (($value_sub['c'] == 1) ? "checked" : "" ); ?> name="submenu[<?= $rand ?>][c]"></td>
													<td><input type="checkbox" class="chk<?= $key_menu?> r<?= $key_menu ?>" <?= (($value_sub['r'] == 1) ? "checked" : "" ); ?> name="submenu[<?= $rand ?>][r]"></td>
													<td><input type="checkbox" class="chk<?= $key_menu?> u<?= $key_menu ?>" <?= (($value_sub['u'] == 1) ? "checked" : "" ); ?> name="submenu[<?= $rand ?>][u]"></td>
													<td><input type="checkbox" class="chk<?= $key_menu?> d<?= $key_menu ?>" <?= (($value_sub['d'] == 1) ? "checked" : "" ); ?> name="submenu[<?= $rand ?>][d]"></td>
												</tr>
											<?php endforeach ?>
												<tr>
